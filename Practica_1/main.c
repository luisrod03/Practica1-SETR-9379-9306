...
La coordinacion de tareas mediante el planificador apropiativo de FreeRTOS permite crear sistemas reactivos 
sin bloquear el procesador en bucles de espera. El uso del Idle Hook demuestra la eficiencia energetica del RTOS, 
aprovechando los tiempos muertos del hardware mientras se garantiza una respuesta inmediata a los eventos criticos 
gracias a una correcta jerarquia de prioridades.
 */
...
void vTaskLedRapido(void *pvParameters) {
    ...
    while(1) {
        if (g_ledRapido) {
            // Imprime cada 5 ciclos (1 segundo) para no saturar
            if (contadorRapido % 5 == 0) {
                printf("[LED_R] Frecuencia Rapida\n");
            }
            ...
            printf("[LED_R] Transicion a Frecuencia Lenta\n");
            vTaskSuspend(NULL); 
        } else {
    ...
    while(1) {
        if (!g_ledRapido) {
            ...
            if (contadorTimeout >= 5) {
                printf("[LED_L] 5 s. Completados. Regresando a Frecuencia Rapida\n");
